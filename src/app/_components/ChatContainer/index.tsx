  ...
  const [messages, setMessages] = useState<
    {
      role: string
      content: string
      products?: Pick<ProductRowSelect, 'id' | 'description' | 'name' | 'images'>[]
    }[]
  >([])
    ...
    <div className={styles['chat-container']}>
      <div style={{ display: 'flex', gap: 12 }}>
        <div className={styles['chat-bubble']}>
          <p className={styles['bot-message']}>I am Avani, how can I help you?</p>
          {messages.map(m => (
            <div key={m.content}>
      ...
      <div className={styles['footer']}>
        ...
        <button
          ...
          onClick={() =>
            utils.ai.getProductSuggestion.fetch({ message: input }).then(res => {
              setInput('')
              ...
              const voice = window.speechSynthesis.getVoices().filter(function (voice) {
