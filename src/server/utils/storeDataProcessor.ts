  ...
  try {
    ...
    await Promise.all(
      _.chunk(embeddingRows, 16).map(async (embeddingSubChunks, i) => {
        const embeddingsObjs = await getEmbeddings(embeddingSubChunks.map(c => c.text!))
...
export async function readJsonlFile() {
  const jsonl = await fetch('http://localhost:3000/assets/data/pilgrim.jsonl').then(res => res.text())
  ...
  for await (const product of products) {
    ...
    if (product.description) description += ` the product description given by the merchant is, \n"${product.title}" \n-----\n\n`
    if (product.category) description += ` the product belongs to the category '${product.category}'\n\n`
    if (product.productType) description += ` the product type is '${product.productType}'\n\n`
    if (product['seo.title']) description += ` the product has SEO title is '${product['seo.title']}'\n\n`
    if (product['seo.description']) description += ` the product has SEO description is '${product['seo.description']}'\n\n`
    if (product.tags?.length) description += ` the product is also tagged with keywords '${product.tags.join(', ')}'\n\n`
    ...
    if (noOfVariants) {
      description += ` the product has ${noOfVariants} variant${pluralize(
        noOfVariants,
      )} and may have one image per variant attached after product image${pluralize(noOfImages)}.`
      description += product.variants.map(v => `'${v.title}'`).join(', ') + ` are the product variants.`
    }

    const productImages = product.images?.map(i => i.imageUrl + '&width=512').slice(0, 3)
    const variantImages = product.variants?.map(i => !!i.imageUrl && i.imageUrl + '&width=512').filter(Boolean) as string[]
    const allProductImages = Array.from(new Set([...productImages, ...variantImages]))
    ...
    try {
      if (!allProductImages.length) throw Error('no product images for vision api!')
      ...
      description += ' make sure extracted information is about the product described above not any other thing or person in images.'
      description += ' description must be very crisp must have all the information from provided description and extracted from image.'
      description += ' also description should be optimized for vector embedding generation for vector search operations.'
      description += ' again, please do not repeat any information and do not miss any information.'
...
const pickRandomPrice = () => {
  const numbers = [149, 199, 249, 299, 349, 399, 449, 499, 549, 599, 649, 699, 749, 799]
